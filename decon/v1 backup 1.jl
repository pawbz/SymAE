...
# ╔═╡ ee79a3cb-96e0-4ddf-9dc5-eb3052b4bb09
using Tullio, Distributions, CUDAKernels, KernelAbstractions

# ╔═╡ 7c9f2512-12aa-11ee-22c6-7f9abc5509a0
using Flux, JLD2, Random, MLUtils, Plots, DSP, ProgressLogging, Statistics, Zygote, LinearAlgebra, PlutoLinks, PlutoUI, PlutoHooks, OneHotArrays, HDF5
...
eqdata = h5open("eq_sample_data_shallow.hdf5", "r");
...
function apply_gaussian3d!(x)
	σ=rand(Uniform(0, 0.5))
	t0=rand(1:size(x,1))
	@tullio X[i,j,k] := x[i,j,k] * exp(-0.5*(abs((i-t0)) * inv(σ)))
	return X
end

# ╔═╡ 367e1ad1-6f63-4210-b35f-00bc2a7f3bdc
joinobs(rand(10,4), rand(10,4)) |> collect

# ╔═╡ f7c7764e-38b5-44e5-9775-17928aaaa106
xx=Flux.normalize(randn(10, 1, 1)); XX=apply_gaussian(xx);plot(xx[:, 1,1]);plot!(XX[:, 1,1])

# ╔═╡ c7f555ab-862f-4085-be68-18b61111c4a1
norm(XX)

# ╔═╡ 1e07db01-f868-4278-9fdd-2357feb5c3d5
xxx=randn(1,6); yy=first(splitobs(xxx, at=0.5)); fill!(yy, 0), print(xxx)

# ╔═╡ 7715f948-2e43-4696-8f99-f4e5675b2d2c
plot(vec(xxx))#xcollect(x)
...
# ╔═╡ f855a34a-9c46-4218-bf46-4eb401153720
reduce(vcat, collect([[1,2,3], [12,3]]))

# ╔═╡ d354d369-7cbb-44d2-8949-e7ec4890a479
@bind iplt2 Slider(1:100, show_value=true)

# ╔═╡ 28b4d791-acd8-4f40-8c34-838af73fbac4
md"""
Gaussian standard deviation
$(@bind σ Select([0.0005, 0.001,  0.005, 0.01, 0.1, 1]))
"""

# ╔═╡ 8949f5e4-0102-4c92-9d13-59e1321a351d
...
# ╔═╡ ee7b69dc-c825-41b4-a65d-da327666c0b8
theme(:juno)

# ╔═╡ 370efc94-165b-48e2-859b-b2c9abfa2622
default(framestyle=:zerolines, minorgrid=true)
...
Deq = map(keys(eqdata)) do eq
    mapreduce(hcat, keys(eqdata[eq])) do pixel
        Flux.normalise(xpu(resample(Array(eqdata[eq][pixel]["data"]), 0.5, dims=1)), dims=1)
    end
end;
...
X = symae.get_data_iterator(D, batchsize=128)

# ╔═╡ 543b3e59-3583-49fd-9900-a55fb80129c3
mp(randobs(randobs(D)))
...
# ╔═╡ bfd2ee1e-78f6-406b-9c8d-79e27b907967
E = map(1:nt) do i
    xpu(cat(onehot(i, 1:nt), dims=3))
end

# ╔═╡ a32830cf-3a01-44eb-8d34-0035fc98240c
begin
    NN = symae.get_dense_networks(nt, p, q2)
...
# ╔═╡ 851c674e-2c13-4670-bd76-da3044e17ebc
begin
	ftest = plot(De, size=(650, 300), w=2, label=permutedims(cat(Dnames, dims=2),(2,1)));
		# plot!(redatumed.d2new[:, iplt2],  c=:crimson, label="Nuisance", ylim=(0, 1))
end
...
# ╔═╡ 0fcd9e51-7990-4b3a-a42c-51808bd6305d
range(-5, 5, length=10) |> collect

# ╔═╡ 79e1ec4c-1724-4cf0-b3a0-d6b67a6c7512
circshift(randn(10), 4.5)

# ╔═╡ 49ed3c9b-1ea1-4615-ad1f-8196289ccf71
function gaussian_windows(nt, nr, σ)
    iss = [randobs(-div(nt, 2):div(nt, 2)) for i in 1:nr]
    iss = sort(iss)
...
# ╔═╡ b5511093-c8a4-4161-9ed8-ca5f4dd75139

begin
    trained

    p1 = plot(redatumed.d2[:, iplt2], c=:lightblue, title="$(Dnames[j]): True Vs. Reconstructed", label=nothing)
    plot!(p1, cpu(redatumed.d2hat[:, iplt2]), c=:lightgreen, label=nothing)

    d2newhat = redatumed.d2newhat[:, iplt2]
    p7 = plot(d2newhat, c=:orange, title="Deconvolved $(Dnames[j])", label=nothing)
    plot!(p1, redatumed.d2new[:, iplt2], c=:crimson, label="Nuisance")
    plot!(p7, abs.(hilbert(d2newhat)), l=(:dot, 1), c=:orange, alpha=0.3, label=nothing)

    d1newhat = redatumed.d1newhat[:, iplt2]
    p4 = plot(d1newhat, c=:orange, title="Deconvolved $(Dnames[i])", label=nothing)
    d1new = redatumed.d1new[:, iplt2]

    plot!(p4, abs.(hilbert(d1newhat)), c=:orange, label=nothing, l=(:dot, 1), alpha=0.3)

    p5 = plot(redatumed.d1[:, iplt2], c=:lightblue, title="$(Dnames[i]): True Vs. Reconstructed", label=nothing)
    plot!(p5, redatumed.d1hat[:, iplt2], c=:lightgreen, label=nothing)
    plot!(p5, d1new, c=:crimson, label="Nuisance")

    plot(p1, p7, p5, p4, layout=(4, 1), size=(800, 800), w=2, ylim=(-8, 8))
end
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "3fc5eb50a51157ad9e1fea9c1fde8f0555108af4"
