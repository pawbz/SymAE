...
using Flux, JLD2, Random, MLUtils, DSP, ProgressLogging, Statistics, Zygote, LinearAlgebra, PlutoLinks, PlutoUI, PlutoHooks, OneHotArrays, HDF5, Tullio, Distributions, CUDAKernels, KernelAbstractions, CUDA, PlutoPlotly, FFTW, ImageFiltering, JuliennedArrays
...
md"""
Redatuming peak $(@bind iplt2 Slider(1:203, show_value=true));
...
# ╔═╡ e6ee0bc7-2bfb-4f00-bf45-7d2f1d1f77b5
plot(angle.(rfft(s2)))

# ╔═╡ c5be4a2b-0390-442c-b30d-8562f871f0d5
plot(remove_linear_phase(s2))
...
# ╔═╡ 089390e2-06e9-4c01-99ed-6ce2449302e1
plot(heatmap(z=Deq_syn[3]))
...
Deq = map(keys(eqdata)) do eq
    mapreduce(hcat, keys(eqdata[eq])) do pixel
        ...
        Flux.normalise(xpu(taper(resampled_zero_lag_data)), dims=1)
    end
end;
...
begin
	...
	NN = symae.get_dense_networks(nt, p, q, nt_senc=nt)
	...
	tencb = symae.BroadcastTenc(NN.tenc)
...
function loss(x)
	Flux.mse(model(x), x) #+ 100.0 * mean(abs2, nencb(x))
end
...
begin
if(augment)
	Daug = augmented_state(Dreal)
	D = vcat(Dreal, Daug)
else
	D = Dreal
end
X = symae.get_data_iterator(D, batchsize=128, ntau=500)
end;

# ╔═╡ 03b389ec-38df-4b85-8463-4d9fea910c5b
plot(cpu(D[1][:, 100]))

# ╔═╡ 89686cfa-4666-4c1e-9801-3cb655bd2468
plot(circshift(remove_linear_phase(cpu(randobs(D[4]))), 0))

# ╔═╡ 543b3e59-3583-49fd-9900-a55fb80129c3
let
	dplot = cpu(randobs(randobs(D),2))
...
# ╔═╡ d9379475-c196-421d-8be2-09337f6def98
Flux.mse(model(first(X)), first(X))
...
function redatum(NN, i, j, σ)
	...
	Di = randobs(D[i], nr)
	Dj = randobs(D[j], nr)
	# compute coherent code, which is repeated, then select only nr
    zsym1 = sencb(D[i])[:, 1:nr]
    zsym2 = sencb(D[j])[:, 1:nr]
    # zsym_new = sencb(dnew)
    d1hat = model(Di)
    d2hat = model(Dj)
    # dnewhat = model(dnew)
...
begin
	trained
	plot_data([redatumed.d1[:, iplt2], redatumed.d1hat[:, iplt2]], title="$(Dnames[i]) reconstruction"; labels=["true", "reconstructed"])
end

# ╔═╡ a21cf4fd-3e86-4ff9-9446-4825bfaedfac
begin
	trained
	plot_data([redatumed.d2[:, iplt2], redatumed.d2hat[:, iplt2]], title="$(Dnames[j]) reconstruction", labels=["true", "reconstructed"])
end
...
let
	...
	plot_data(dvec, title="$(Dnames[i]) deconvolved", labels=["true", "deconvolved", "redatuming peak"], ylims=ylims, opacitys=[0.5,1,0.5])
end
...
let
	...
	ylims = penvelope ? (0,10) : (-8,8)
	plot_data(dvec, title="$(Dnames[j]) deconvolved", labels=["true", "deconvolved", "redatuming peak"], ylims=ylims, opacitys=[0.5,1,0.5])
end
...
plot(real.(ifft(fft(cpu(redatumed.d1newhat)[:, iplt2]) ./ (1f-10 .+ fft(g2[:, iplt2])))))
...
# ╔═╡ 97895724-5b2f-48bf-a136-8d5c3a92afa4
test=cat(my_gaussian(nt, 0.01), dims=3);
...
plot(apply_random_time_shifts!(test, -50:50)[:, 1, 1])

# ╔═╡ e7220b14-bcc1-4045-b17c-b41bd3d68a45
apply_random_time_shifts!(test, -50:50) == test
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "e6e9235df17c379104dbcc0bb58891b932949c91"
